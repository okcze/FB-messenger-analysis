...
#Sample data reading
most_popular_both <- read.csv("./sample_data/most_popular_both_Piotr.csv", encoding = "UTF-8")
spedzony_czas <- read.csv("./sample_data/spedzony_czas_Piotr.csv", encoding = "UTF-8")
spedzony_czas_osoby <- read.csv("./sample_data/spedzony_czas_osoby_Piotr.csv", encoding = "UTF-8")
month_message_all <- read.csv("./sample_data/month_all_Piotr.csv", encoding = "UTF-8")
hour_message_all <- read.csv("./sample_data/hour_all_Piotr.csv", encoding = "UTF-8")
day_message_all <- read.csv("./sample_data/day_all_Piotr.csv", encoding = "UTF-8")
df_joychart <- read.csv("./sample_data/joy_chart_Piotr.csv", encoding = "UTF-8")
...
server <- function(input, output){
  ...
  output[["wordcloud"]] <- renderWordcloud2({
    ...
    for (osoba in lista_osob) {
      ...
      current_nazwa <- paste("./sample_data/wordcloud/wordcloud_",aktualna_osoba,".csv",sep="")
  ...
  output[["joychart"]] <- renderPlot({
    ...
    df_joychart %>% filter(rozmowca %in% lista_osob)%>%
      ...
      geom_vline(xintercept = c(a,b), linetype="dashed", color=kolory[length(lista_osob)], size=1)+
