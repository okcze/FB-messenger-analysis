...
#Do ustawienia
setwd("")
df <- read.csv("/messenger.csv", encoding="UTF-8") #csv ze skryptu python
#
...
## Osoby z ktorymi najwiecej piszemy (wyslane + odebrane)
...
## Ilosc wiadomsoci w dane dni tygodnia (wyslane + odebrane)

timestamp <- strptime(df1$timestamp_ms, "%Y-%m-%d %H:%M:%S") #wektor dat ale w POSIX
...
## ilosc wiadomosci w z podzialem na miesiace (wyslane + odebrane)
...
## ilosc wiadomosci w z podzialem na godziny (wyslane + odebrane) 0 oznacza 00.00 - 01.00, 1 oznacza 01.00 - 02.00 itd.
...
## srednia ilosc wiadomosci w z podzialem na miesiace (wyslane + odebrane)
...
## srednia ilosc wiadomsoci w dane dni tygodnia (wyslane + odebrane)
